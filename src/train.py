...
'''Training
parameter setting in sun.sh
Directly run sh run.sh in command line

SEQ_LEN<model parameter>: the volume of context for considering
SHOT_NUM<model parameter>: the length of frame for considering
SIM_CHANNEL<model parameter>: the number of channels for conv2d
RATIO<model parameter>: the importance of place features, cast features, action features, audio features
NUM_LAYERS<model parameter>: number of layers of LSTM
LSTM_HIDDEN_SIZE<model parameter>: the hidden size of LSTM
BIDIRECTIONAL<model parameter>: bidirectional or not of LSTM
MAX_LEN<dataset parameter>: the max len of film with padding and (max_len mode SEQ_LEN == 1)
TRAIN_BATCH_SIZE<dataset parameter>: batch size for training
VALID_BATCH_SIZE<dataset parameter>: batch size for validation
VALID_PATH<dataset parameter>: validation path
EPOCHS: epochs for training
LR: initial training rate
OPT: the number of epoch to reduce learning rate by 10
THRESHOLD<evaluation parameters>: a threshold to filter the prediction
'''
...
def run():
    ''' Training
    <Ruturns>:
        dfhistory : training history    '../input/models/dfhistory.csv'
        models:  model zoo '../input/model/SenceSegNet_epoch*.bin'
    '''
    ...
    for epoch in range(1, EPOCHS + 1):
        if epoch == OPT: # change learning rate(mentioned in paper)
