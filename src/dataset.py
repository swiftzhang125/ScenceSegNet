...
class SceneSegDataset():
    ...
    Args:
        ...
        max_len: the max len of film with padding and (max_len mode 10 == 1)
    Returns:
        a iterator for dataloader
            'place_feats': place features with zero padding
            'cast_feats': cast features with zero padding
            'action_feats': action features with zero padding
            'audio_feats': audio features with zero padding
            'targets': ground-truth with zero padding
            'masks': original data for [1], padding for [0]
            'imdb_id': imdb_id
    <padding helps for batch training>
    '''
...
'''
Check the result of SceneSegDataset class
Don't need to run
'''
